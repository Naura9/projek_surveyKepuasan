<?php
    ...
    $query_get_responden_id = "SELECT responden_alumni_id, survey_id FROM t_responden_alumni WHERE responden_nama = '$nama'";
    ...
    $responden_alumni_id = $row_get_responden_id['responden_alumni_id'];
    ...
    $query_check_survey = "SELECT COUNT(*) AS jumlah_survey FROM t_jawaban_alumni
                            JOIN m_survey_soal ON t_jawaban_alumni.soal_id = m_survey_soal.soal_id
                            WHERE m_survey_soal.kategori_id = 3
                            AND t_jawaban_alumni.responden_alumni_id = '$responden_alumni_id'";
    ...
    if(isset($_POST['simpan'])) {
        $username = $_SESSION['username']; 
        $nama = $_SESSION['nama']; 
        $query_responden = "SELECT responden_alumni_id FROM t_responden_alumni WHERE responden_nama = '$nama'";
        ...
        if(mysqli_num_rows($result_responden) > 0) {
            ...
            $responden_alumni_id = $data_responden['responden_alumni_id'];
        } else {
        ...
        while ($row = mysqli_fetch_assoc($result_soal_id)) {
            ...
            $query_insert_jawaban = "INSERT INTO t_jawaban_alumni (responden_alumni_id, soal_id, jawaban) 
                                     VALUES ('$responden_alumni_id', '$soal_id', '$jawaban')";
        ...
        $query_update_tanggal = "UPDATE t_responden_alumni SET responden_tanggal = CURDATE() WHERE responden_alumni_id = '$responden_alumni_id'";
